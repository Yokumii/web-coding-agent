...
def _build_generate_prompt(
    *,
    ...
    return (
        ...
        f"For Bash, command chains and pipelines are allowed when each segment stays inside the workdir.\n"
        f"Do not use background execution, redirection, or command substitution such as `&`, `>`, `<`, `$(`, or backticks.\n"
        f"For package-manager and build commands, target `frontend/` explicitly with "
        f"`npm --prefix frontend ...` or `cd frontend && ...`; never run `npm run build` from the workdir root.\n"
