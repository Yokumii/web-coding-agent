...
- [references/authentication.css](references/authentication.css)
- [references/authentication.js](references/authentication.js)

直接复制并引用参考文件，不让模型重写一份实现。沿用当前项目的目录、模块和样式规范；代码不依赖 Skill 目录、Harness 或外部包。组件正确性由开发侧测试维护，当前 Edit 仍使用既有 Harness 检查。

## 接入位置

现有登录/注册区域、会话服务和登录态消费者（导航、用户信息或受限入口）。

只改当前指令允许的区域，不造独立 demo，不以类别名为由添加无关功能。若现有事件已承担同一业务，应替换相应旧处理，避免重复更新。

## 最小适配

将 `references/authentication.js` 与同目录 CSS（如有）直接复制到当前项目的组件目录。普通 HTML 使用现有 script/link 加载方式；ES Module 项目只给入口函数增加 export 并按现有方式 import。在宿主区域挂载后调用，视图卸载时调用 destroy；样式通过组件作用域内的颜色、字体、间距覆盖适配。不要重写状态或核心算法。

入口：`mountAuthentication({ ... })`。

参数：`container, authenticate, restoreSession?, signOut?, onSession, allowRegistration = false`。

authenticate(credentials, {mode, signal}) 返回用户身份；restoreSession({signal}) 返回身份或 null；signOut({signal}) 清理宿主会话；onSession 更新现有导航。组件负责表单、pending/错误、登录/退出同步；注册仅按指令开启。

组件不存密码、不伪造安全认证，不默认使用 localStorage。真实权限由服务端负责；模拟服务必须明确标注。destroy() 作废在途结果，避免离开页面后更新 UI。

宿主适配示例（变量映射到当前源码已有对象）：

```javascript
const component = mountAuthentication({
  container: accountRegion,
  authenticate: (credentials, options) => existingAuth.signIn(credentials, options),
  restoreSession: options => existingAuth.currentUser(options),
  signOut: options => existingAuth.signOut(options),
  onSession: user => updateExistingAccountNavigation(user)
}); // 根据现有服务协议适配；authenticate 返回 {name?, email?}，至少有一项非空
```
...
- React / TypeScript / Vite：[Component.jsx](references/Component.jsx) + [authentication.mjs](references/authentication.mjs) + 同目录 CSS；在现有组件中引入，不改构建配置。TypeScript 项目沿用已有 JS 互操作规范。
...
Submit invalid credentials and assert failure; submit valid credentials via real or explicitly simulated auth and assert identity/navigation state; sign out and assert cleared session. Refresh only when persistence is required.
