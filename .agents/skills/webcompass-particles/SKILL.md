...
- [references/particles.js](references/particles.js)

直接复制并引用参考文件，不让模型重写一份实现。沿用当前项目的目录、模块和样式规范；代码不依赖 Skill 目录、Harness 或外部包。组件正确性由开发侧测试维护，当前 Edit 仍使用既有 Harness 检查。

## 接入位置

当前 Hero 或指定背景区域、真实尺寸、现有主色和需要保留点击的前景内容。

只改当前指令允许的区域，不造独立 demo，不以类别名为由添加无关功能。若现有事件已承担同一业务，应替换相应旧处理，避免重复更新。

## 最小适配

将 `references/particles.js` 与同目录 CSS（如有）直接复制到当前项目的组件目录。普通 HTML 使用现有 script/link 加载方式；ES Module 项目只给入口函数增加 export 并按现有方式 import。在宿主区域挂载后调用，视图卸载时调用 destroy；样式通过组件作用域内的颜色、字体、间距覆盖适配。不要重写状态或核心算法。

入口：`mountParticles({ ... })`。

参数：`container, count = 45, color, linkDistance = 90, interaction = true`。

组件添加不拦截点击的 Canvas，负责粒子漂浮、邻近连线、指针排斥、点击扩散、尺寸变化和可见性暂停。只映射宿主区域、颜色和合理粒子数量。

count 限制 1..300；高密度连线为二次复杂度。Canvas 不承载关键业务信息；尊重 reduced motion，destroy() 清理 RAF、观察器及监听器。

宿主适配示例（变量映射到当前源码已有对象）：

```javascript
const component = mountParticles({
  container: existingHero,
  color: existingAccentColor,
  count: 45
});
```
...
- React / TypeScript / Vite：[Component.jsx](references/Component.jsx) + [particles.mjs](references/particles.mjs) + 同目录 CSS；在现有组件中引入，不改构建配置。TypeScript 项目沿用已有 JS 互操作规范。
...
Observe successive rendered frames for motion within the target canvas, interact where requested, and verify underlying controls remain clickable. Canvas presence alone does not prove animation.
