...
- [references/wizard.js](references/wizard.js)

直接复制并引用参考文件，不让模型重写一份实现。沿用当前项目的目录、模块和样式规范；代码不依赖 Skill 目录、Harness 或外部包。组件正确性由开发侧测试维护，当前 Edit 仍使用既有 Harness 检查。

## 接入位置

同一个现有 form 内的步骤区域、真实输入字段、提交逻辑和用户可理解的步骤名称。

只改当前指令允许的区域，不造独立 demo，不以类别名为由添加无关功能。若现有事件已承担同一业务，应替换相应旧处理，避免重复更新。

## 最小适配

将 `references/wizard.js` 与同目录 CSS（如有）直接复制到当前项目的组件目录。普通 HTML 使用现有 script/link 加载方式；ES Module 项目只给入口函数增加 export 并按现有方式 import。在宿主区域挂载后调用，视图卸载时调用 destroy；样式通过组件作用域内的颜色、字体、间距覆盖适配。不要重写状态或核心算法。

入口：`mountWizard({ ... })`。

参数：`form, steps: [{element, label}], onComplete`。

直接传入已有步骤元素，不复制输入或重建表单。组件负责当前步、验证前进条件、返回保留、进度和完成提交；onComplete(formData) 连接现有提交或最终结果。

控件必须有 name；同名多值保留为数组。文件上传值与跨字段复杂规则需按当前需求局部适配。组件接管此 form 的步骤提交，因此应替换旧提交处理，避免重复提交。

宿主适配示例（变量映射到当前源码已有对象）：

```javascript
const component = mountWizard({
  form: existingForm,
  steps: existingStepElements.map((element, i) => ({element, label: stepLabels[i]})),
  onComplete: formData => existingSubmit(formData)
});
```
...
- React / TypeScript / Vite：[Component.jsx](references/Component.jsx) + [wizard.mjs](references/wizard.mjs) + 同目录 CSS；在现有组件中引入，不改构建配置。TypeScript 项目沿用已有 JS 互操作规范。
...
Attempt advancing with missing required fields, then fill and advance; go back and assert preserved values; finish and assert actual submitted result with values from all steps.
