...
- [references/notifications.css](references/notifications.css)
- [references/notifications.js](references/notifications.js)

直接复制并引用参考文件，不让模型重写一份实现。沿用当前项目的目录、模块和样式规范；代码不依赖 Skill 目录、Harness 或外部包。组件正确性由开发侧测试维护，当前 Edit 仍使用既有 Harness 检查。

## 接入位置

已有事件/消息来源、通知入口、实际操作目标以及未读提示位置。

只改当前指令允许的区域，不造独立 demo，不以类别名为由添加无关功能。若现有事件已承担同一业务，应替换相应旧处理，避免重复更新。

## 最小适配

将 `references/notifications.js` 与同目录 CSS（如有）直接复制到当前项目的组件目录。普通 HTML 使用现有 script/link 加载方式；ES Module 项目只给入口函数增加 export 并按现有方式 import。在宿主区域挂载后调用，视图卸载时调用 destroy；样式通过组件作用域内的颜色、字体、间距覆盖适配。不要重写状态或核心算法。

入口：`mountNotificationCenter({ ... })`。

参数：`container, notifications: [{id, message, read?, actionLabel?}], allowDelete = false, onAction, onChange, onCreateRow`。

push(notification) 接入真实到达事件；组件维护未读数量、列表、单条/全部已读及按需删除；onAction(notification) 连接当前业务目标。snapshot() 返回独立通知数据。

持久化历史时，新事件 ID 必须跨刷新唯一（如 crypto.randomUUID）；同一业务事件的重试保留身份，用事件 ID 与状态去重。只在已接好真实 onAction 时传 actionLabel。要求日期分组时显示日期标题，data-date 属性本身不构成分组。

同一通知 ID 不能重复插入；持久化与服务器 read 状态属于宿主接口，按指令接线。没有默认随机通知、弹窗或存储。

宿主适配示例（变量映射到当前源码已有对象）：

```javascript
const component = mountNotificationCenter({
  container: notificationRegion,
  notifications: existingNotifications,
  onAction: notification => openExistingTarget(notification.id)
});
// 在现有新通知事件中调用 component.push(notification)。
```

销毁该区域时调用返回对象的 `destroy()`。需要持久化、接口模拟或额外操作时，只接入当前指令明确要求的部分；额外需求未实现时不能把整条 Edit 标成完成。

宿主接线：`onChange(entries, unread)` 同步现有 HUD 徽标和按需持久化；`onCreateRow(row, entry)` 只补标题、时间、图标、分组等指令要求的显示。核心保留条目额外 metadata，统一维护 read 和计数。返回 `root/count/list/all` 可设置宿主标签和样式；使用 `push()` 接入现有业务事件，事件 ID 去重由宿主在派发前判断。不要另建一份 read 状态或另算未读数。

挂载时会立即调用 onChange/onCreateRow，回调应使用传入的数据和已存在的宿主 DOM，不要引用尚未赋值的 component 常量。事件适配在真实业务成功/失败的分支里 push；挂载、render、打开面板本身不能重复派发业务通知。打开面板保留既有 read，只有明确的已读操作才改变它。持久化时保存传入的 entries；面板的 hidden 状态需有作用域内 CSS 保证，不能被 display:flex/grid 覆盖。

## 技术栈选择与直接复用

- 原生 HTML/CSS/JS：保留已有 script/link，直接复用本目录经典 JS 与 CSS。
- React / TypeScript / Vite：[Component.jsx](references/Component.jsx) + [notifications.mjs](references/notifications.mjs) + 同目录 CSS；在现有组件中引入，不改构建配置。TypeScript 项目沿用已有 JS 互操作规范。
...
Start from known unread notifications, mark one read and assert exact count decrement and row state; activate its business action. Test mark-all/delete/new arrival when requested.
