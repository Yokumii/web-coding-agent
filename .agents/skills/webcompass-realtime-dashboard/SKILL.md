...
- [references/dashboard.css](references/dashboard.css)
- [references/dashboard.js](references/dashboard.js)

直接复制并引用参考文件，不让模型重写一份实现。沿用当前项目的目录、模块和样式规范；代码不依赖 Skill 目录、Harness 或外部包。组件正确性由开发侧测试维护，当前 Edit 仍使用既有 Harness 检查。

## 接入位置

已有指标卡片及指标来源；确认各数字、趋势图所表达的是同一批数据。

只改当前指令允许的区域，不造独立 demo，不以类别名为由添加无关功能。若现有事件已承担同一业务，应替换相应旧处理，避免重复更新。

## 最小适配

将 `references/dashboard.js` 与同目录 CSS（如有）直接复制到当前项目的组件目录。普通 HTML 使用现有 script/link 加载方式；ES Module 项目只给入口函数增加 export 并按现有方式 import。在宿主区域挂载后调用，视图卸载时调用 destroy；样式通过组件作用域内的颜色、字体、间距覆盖适配。不要重写状态或核心算法。

入口：`mountRealtimeDashboard({ ... })`。

参数：`container, metrics: [{key, label, format?}], fetchSample?, intervalMs = 3000, historyLimit = 30`。

现有异步数据源映射为 fetchSample({signal})，返回以 metric.key 为键的数值对象；事件驱动项目可省略轮询并调用 update(sample)。同次更新同步数字、SVG 趋势及时间戳。若状态行还包含 Live/暂停/恢复按钮、时间戳或其他控制节点，必须保留这些子节点；动态提示使用独立的 status message 节点，只更新该节点，不要用 `status.textContent = ...` 覆盖整个状态容器。

没有内置随机假数据；只有指令明确要求模拟时才在宿主回调提供标明模拟的数据。失败保留上次有效值；不并发轮询，destroy() 中止请求和定时器。

宿主适配示例（变量映射到当前源码已有对象）：

```javascript
const component = mountRealtimeDashboard({
  container: metricsRegion,
  metrics: [{key: "orders", label: "Orders"}],
  fetchSample: ({signal}) => existingMetricsService({signal}),
  intervalMs: 3000
});
```
...
- React / TypeScript / Vite：[Component.jsx](references/Component.jsx) + [dashboard.mjs](references/dashboard.mjs) + 同目录 CSS；在现有组件中引入，不改构建配置。TypeScript 项目沿用已有 JS 互操作规范。
...
Obtain two distinct samples from the specified real or explicitly simulated data source; assert changed displayed values and trend output. A static number or timer label is insufficient.
