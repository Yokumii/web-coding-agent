...
- [references/skeleton.css](references/skeleton.css)
- [references/skeleton.js](references/skeleton.js)

直接复制并引用参考文件，不让模型重写一份实现。沿用当前项目的目录、模块和样式规范；代码不依赖 Skill 目录、Harness 或外部包。组件正确性由开发侧测试维护，当前 Edit 仍使用既有 Harness 检查。

## 接入位置

当前异步内容区域、最终内容的真实布局，以及匹配标题/图片/段落尺寸的占位结构。

只改当前指令允许的区域，不造独立 demo，不以类别名为由添加无关功能。若现有事件已承担同一业务，应替换相应旧处理，避免重复更新。

## 最小适配

将 `references/skeleton.js` 与同目录 CSS（如有）直接复制到当前项目的组件目录。普通 HTML 使用现有 script/link 加载方式；ES Module 项目只给入口函数增加 export 并按现有方式 import。在宿主区域挂载后调用，视图卸载时调用 destroy；样式通过组件作用域内的颜色、字体、间距覆盖适配。不要重写状态或核心算法。

入口：`mountSkeletonLoading({ ... })`。

参数：`container, skeleton, load, renderContent, duration = 180`。

skeleton 是与目标结构匹配的 Element；用 data-skeleton-block 标记占位块。load({signal}) 返回真实数据；renderContent(data) 沿用现有布局返回 Element。组件负责占位、加载成功切换、失败与重试。

挂载区域应为空；图片/卡片尺寸由宿主保持一致，不能用随意灰块冒充匹配结构。没有固定假延时或默认网络请求。destroy() 中止请求并移除自有节点。

宿主适配示例（变量映射到当前源码已有对象）：

```javascript
const component = mountSkeletonLoading({
  container: emptyContentRegion,
  skeleton: existingSkeletonTemplate.content.firstElementChild,
  load: ({signal}) => existingContentService({signal}),
  renderContent: data => renderExistingContent(data)
}); // skeleton 与最终内容尺寸一致；renderContent 返回 Element
```
...
- React / TypeScript / Vite：[Component.jsx](references/Component.jsx) + [skeleton.mjs](references/skeleton.mjs) + 同目录 CSS；在现有组件中引入，不改构建配置。TypeScript 项目沿用已有 JS 互操作规范。
...
With a controlled slow response, assert matching skeleton visible while content is pending, then skeleton removed and actual content shown. Exercise failed load/retry if requested.
