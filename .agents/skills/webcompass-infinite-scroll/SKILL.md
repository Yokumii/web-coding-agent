...
- [references/infinite-scroll.js](references/infinite-scroll.js)

直接复制并引用参考文件，不让模型重写一份实现。沿用当前项目的目录、模块和样式规范；代码不依赖 Skill 目录、Harness 或外部包。组件正确性由开发侧测试维护，当前 Edit 仍使用既有 Harness 检查。

## 接入位置

真实列表、下一批数据来源、稳定条目 ID，以及当前滚动容器。

只改当前指令允许的区域，不造独立 demo，不以类别名为由添加无关功能。若现有事件已承担同一业务，应替换相应旧处理，避免重复更新。

## 最小适配

将 `references/infinite-scroll.js` 与同目录 CSS（如有）直接复制到当前项目的组件目录。普通 HTML 使用现有 script/link 加载方式；ES Module 项目只给入口函数增加 export 并按现有方式 import。在宿主区域挂载后调用，视图卸载时调用 destroy；样式通过组件作用域内的颜色、字体、间距覆盖适配。不要重写状态或核心算法。

入口：`mountInfiniteScroll({ ... })`。

参数：`container, loadPage, getId, renderItem, root = null, firstPage = 1`。

可选参数：`initialLoad: true` 挂载后立即请求首屏；`endText` 设置结束文案；`renderLoading()` 返回骨架屏 Element，由组件在请求期间显示、完成后移除。宿主只需接入数据与样式。

loadPage(page, {signal}) 返回 {items, hasMore}；renderItem(item) 用当前卡片结构返回一个新的 Element。组件负责滚动触发、请求串行化、追加去重、失败重试和结束状态。

不要把全部假数据嵌入组件。已有初始列表需从真实下一页开始并避免重复宿主条目；最简接入用空的目标列表。没有新增记录却声称 hasMore 的响应视为错误。

宿主适配示例（变量映射到当前源码已有对象）：

```javascript
const component = mountInfiniteScroll({
  container: emptyListRegion,
  loadPage: (page, {signal}) => existingListService({page, signal}),
  getId: item => String(item.id),
  renderItem: item => renderExistingCard(item)
}); // service 返回 {items, hasMore}；renderExistingCard 返回新的 Element
```
...
- React / TypeScript / Vite：[Component.jsx](references/Component.jsx) + [infinite-scroll.mjs](references/infinite-scroll.mjs) + 同目录 CSS；在现有组件中引入，不改构建配置。TypeScript 项目沿用已有 JS 互操作规范。
...
Record initial items, scroll to the sentinel, assert new unique items while old items remain; reach the end or a bounded failure/retry state as required.
