...
- [references/validation.js](references/validation.js)

直接复制并引用参考文件，不让模型重写一份实现。沿用当前项目的目录、模块和样式规范；代码不依赖 Skill 目录、Harness 或外部包。组件正确性由开发侧测试维护，当前 Edit 仍使用既有 Harness 检查。

## 接入位置

已有 form、需要异步检查的输入和提交按钮；保留现有 required/type/pattern 等约束。

只改当前指令允许的区域，不造独立 demo，不以类别名为由添加无关功能。若现有事件已承担同一业务，应替换相应旧处理，避免重复更新。

## 最小适配

将 `references/validation.js` 与同目录 CSS（如有）直接复制到当前项目的组件目录。普通 HTML 使用现有 script/link 加载方式；ES Module 项目只给入口函数增加 export 并按现有方式 import。在宿主区域挂载后调用，视图卸载时调用 destroy；样式通过组件作用域内的颜色、字体、间距覆盖适配。不要重写状态或核心算法。

入口：`bindAsyncValidation({ ... })`。

参数：`form, input, submitButton, check, debounceMs = 300, canSubmit = () => true`。

check(value, {signal}) 返回 {valid, message?}；按真实接口或明确模拟规则判断。组件负责防抖、pending/valid/invalid、旧响应作废及提交限制。多个字段可分别绑定同一个 form 和提交按钮。

同一输入只绑定一次。额外业务提交条件通过 canSubmit 提供，不能只依赖按钮 disabled；组件也会拦截未就绪提交。destroy() 恢复关联属性和提交状态。

宿主适配示例（变量映射到当前源码已有对象）：

```javascript
const component = bindAsyncValidation({
  form: existingForm,
  input: existingEmailInput,
  submitButton: existingSubmitButton,
  check: (email, {signal}) => existingEmailService(email, {signal})
}); // service 返回 {valid: boolean, message?: string}
```
...
- React / TypeScript / Vite：[Component.jsx](references/Component.jsx) + [validation.mjs](references/validation.mjs) + 同目录 CSS；在现有组件中引入，不改构建配置。TypeScript 项目沿用已有 JS 互操作规范。
...
Fill an invalid value, assert invalid feedback and blocked submission; fill a valid value, wait for validation and assert enabled submission. Rapidly replace the value and verify stale response cannot win.
