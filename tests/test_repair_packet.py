...
def test_repair_packet_includes_failed_source_edit_risk_evidence(tmp_path):
    harness = tmp_path / ".harness"
    harness.mkdir()
    (harness / "hidden_oracle_evidence_round_2.json").write_text(json.dumps({
        "checks": [{
            ...
            "steps": [{
                ...
                "ok": False,
                "output": {"actual": {"issues": [{"kind": "viewport-horizontal-overflow"}]}},
            }],
        }]
    }))
    ...
    packet = write_repair_packet(
        ...
        sprint_num=1,
        grades={
            "bugs_found": ["The edited catalog overflows the mobile viewport."],
            "repair_instructions": ["Repair the observed Overflow on the catalog."],
        },
    )

    assert packet["status"] == "repairable"
    assert [item["check_id"] for item in packet["failed_checks"]] == [
        "RISK-01-OVERFLOW"
    ]
    assert ".harness/hidden_oracle_evidence_round_2.json" in packet["evidence_refs"]
