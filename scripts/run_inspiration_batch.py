...
def main():
    ...
    def worker(index, row):
        ...
        command = [sys.executable, str(Path(__file__).resolve()), '--case-worker',
                   ...
                   '--run-dir', str(case/'miner'), '--stage', 'full', '--exploration-rounds', '1',
                   '--max-paths', '3', '--max-actions', '4', '--api-timeout-seconds', '90',
                   '--source-mode', 'reference', '--max-source-regions', '4', '--product-patterns']
